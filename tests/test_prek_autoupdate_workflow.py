...
class FakeCleanupClient:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            branch_refs: Branch refs to return for matching branch lookups.
            fail_on_close: Whether closing a PR should fail the test.
        """
        self.open_pulls = open_pulls
        self.closed_pulls = closed_pulls
        self.branch_refs = [] if branch_refs is None else branch_refs
    ...
    def list_refs(self, *, ref_prefix: str) -> list[str]:
        ...
        return [ref for ref in self.branch_refs if ref.startswith(f"refs/{ref_prefix}")]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("--keep-latest-open-pr", "preserves the current update PR during nightly cleanup"),
        ("--delete-stale-branches", "deletes orphaned workflow branches"),
...
def test_workflow_uses_plural_stale_branch_cleanup_only() -> None:
    """Workflow should use prefix branch cleanup instead of fixed branch cleanup."""
...
def test_workflow_runs_updates_weekly_and_cleanup_daily() -> None:
    """Workflow should only update prek weekly while running cleanup every night."""
    workflow_text = WORKFLOW_PATH.read_text()
...
def test_cleanup_script_can_keep_latest_open_workflow_pr(cleanup_script: ModuleType) -> None:
    ...
    assert result.deleted_branches == [
        f"{WORKFLOW_BRANCH}-merged",
        f"{WORKFLOW_BRANCH}-old",
    ]


def test_cleanup_script_deletes_orphaned_workflow_branches(cleanup_script: ModuleType) -> None:
